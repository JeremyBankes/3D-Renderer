...
import math.Glm;
import math.matrix.mat3.Matrix3;
 ...
 * @author GBarbieri
 */
public class Matrix4 extends matrixQuery {
    ...
    // TODO transpose
    public Matrix4(float[] f, int offset) {
        this(
    ...
    public Matrix4(double m00, double m01, double m02, double m03, double m10, double m11, double m12, double m13,
    ...
    public Matrix4 set(double m00, double m01, double m02, double m03, double m10, double m11, double m12, double m13,
            double m20, double m21, double m22, double m23, double m30, double m31, double m32, double m33) {
    ...
    public Matrix4 set(float m00, float m01, float m02, float m03, float m10, float m11, float m12, float m13,
            float m20, float m21, float m22, float m23, float m30, float m31, float m32, float m33) {
    ...
    public Matrix4 set(float[] f, int offset) {
        set(
                f[offset + 0], f[offset + 1], f[offset + 2], f[offset + 3],
                f[offset + 4], f[offset + 5], f[offset + 6], f[offset + 7],
                f[offset + 8], f[offset + 9], f[offset + 10], f[offset + 11],
    ...
    public Matrix4 cleanTranslation() {
        m03 = 0.0f;
        m13 = 0.0f;
        m23 = 0.0f;
        m33 = 1.0f;
    ...
    public Vector4 mul_(Vector4 v) {
        return mul(v, new Vector4());
    }

    public Vector4 mul(Vector4 right, Vector4 res) {
    ...
    public Matrix4 mul_(Matrix4 right) {
        return mul(right, new Matrix4());
    }
    ...
    public static Matrix4 cast_(Quat q) {
        return cast(q, new Matrix4());
    }

    public static Matrix4 cast(Quat q, Matrix4 res) {
    ...
    public boolean equals3(Matrix4 other) {
        return equals3(other, 2);
    }

    public boolean equals3(Matrix4 other, int maxUlps) {
    ...
    public boolean equals(Matrix4 other, int maxUlps) {
        if (!Glm.compareFloatEquals(m00, other.m00, maxUlps)) {
            return false;
        }
        if (!Glm.compareFloatEquals(m01, other.m01, maxUlps)) {
    ...
    public Matrix3 toMat3_() {
        return toMat3(new Matrix3());
    }

    public Matrix3 toMat3(Matrix3 res) {
        return res.set(
                m00, m01, m02, 
                m10, m11, m12, 
                m20, m21, m22);
     ...
     * @return
     */
    public float[] toArray(float[] res, int index) {
        res[index + 0] = m00;
        res[index + 1] = m01;
        res[index + 2] = m02;
        res[index + 3] = m03;
    ...
    public ByteBuffer toDbb_() {
        return Matrix4.this.toDbb(ByteBuffer.allocateDirect(SIZE).order(ByteOrder.nativeOrder()));
    }
    ...
    public void print(String title, boolean outStream) {
		...
		int[] minWidths = new int[4];
		for (int i = 0; i < 4; i++) {
			for (int j = 0; j < 4; j++) {
				int len = String.valueOf(matrix[j + i * 4]).length();
				if (minWidths[j] < len)
					minWidths[j] = len;
			}
		}
		for (int i = 0; i < 4; i++) {
			builder.append("| ");
			for (int j = 0; j < 4; j++) {
				int width = minWidths[j];
				String value = String.valueOf(matrix[j + i * 4]);
				for (int k = 0; k < width - value.length(); k++) {
					builder.append(' ');
				}
				builder.append(value);
				builder.append(j >= 4 - 1 ? ' ' : ", ");
			}
			builder.append("|\n");
		}
