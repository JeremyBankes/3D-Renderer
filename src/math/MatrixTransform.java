...
import math.matrix.mat4.d.Mat4d;
...
abstract class MatrixTransform extends funcMatrix {
    ...
    public static Matrix4 lookAt_(Vector3 eye, Vector3 center, Vector3 up) {
        return GLM_LEFT_HANDED ? lookAtLH(eye, center, up, new Matrix4()) : lookAtRH(eye, center, up, new Matrix4());
    }

    public static Matrix4 lookAt(Vector3 eye, Vector3 center, Vector3 up, Matrix4 res) {
        return GLM_LEFT_HANDED ? lookAtLH(eye, center, up, res) : lookAtRH(eye, center, up, res);
    }

    private static Matrix4 lookAtLH(Vector3 eye, Vector3 center, Vector3 up, Matrix4 res) {
    ...
    private static Matrix4 lookAtRH(Vector3 eye, Vector3 center, Vector3 up, Matrix4 res) {
        // f(normalize(center - eye))
        float fX = center.x - eye.x;
        float fY = center.y - eye.y;
        float fZ = center.z - eye.z;
    ...
    public static Matrix4 ortho_(float left, float right, float bottom, float top) {
        return ortho(new Matrix4(), left, right, bottom, top);
    }
    public static Matrix4 ortho(Matrix4 res, float left, float right, float bottom, float top) {
        res.identity();
    ...
    public static Matrix4 ortho_(float left, float right, float bottom, float top, float zNear, float zFar) {
        return GLM_LEFT_HANDED ? orthoLH(new Matrix4(), left, right, bottom, top, zNear, zFar)
                : orthoRH(new Matrix4(), left, right, bottom, top, zNear, zFar);
    }

    public static Matrix4 ortho(Matrix4 res, float left, float right, float bottom, float top, float zNear, float zFar) {
        return GLM_LEFT_HANDED ? orthoLH(res, left, right, bottom, top, zNear, zFar)
                : orthoRH(res, left, right, bottom, top, zNear, zFar);
    }

    private static Matrix4 orthoLH(Matrix4 res, float left, float right, float bottom, float top, float zNear, float zFar) {
    ...
    private static Matrix4 orthoRH(Matrix4 res, float left, float right, float bottom, float top, float zNear, float zFar) {
        res.m00 = 2.0f / (right - left);
        res.m01 = 0.0f;
        res.m02 = 0.0f;
        res.m03 = 0.0f;
    ...
    public static Matrix4 perspective_(float fovy, float aspect, float zNear, float zFar) {
        return GLM_LEFT_HANDED ? perspectiveLH(fovy, aspect, zNear, zFar, new Matrix4())
                : perspectiveRH(fovy, aspect, zNear, zFar, new Matrix4());
    }

    public static Matrix4 perspective(float fovy, float aspect, float zNear, float zFar, Matrix4 mat) {
        return GLM_LEFT_HANDED ? perspectiveLH(fovy, aspect, zNear, zFar, mat)
                : perspectiveRH(fovy, aspect, zNear, zFar, mat);
    }

    private static Matrix4 perspectiveRH(float fovy, float aspect, float zNear, float zFar, Matrix4 res) {
    ...
    private static Matrix4 perspectiveLH(float fovy, float aspect, float zNear, float zFar, Matrix4 res) {
        float tanHalfFovy = (float) Math.tan(fovy * 0.5f);
        res.m00 = 1.0f / (aspect * tanHalfFovy);
        res.m01 = 0.0f;
        res.m02 = 0.0f;
    ...
    public static Matrix4 perspectiveFov_(float fov, float width, float height, float zNear, float zFar) {
        return GLM_LEFT_HANDED ? perspectiveFovLH(fov, width, height, zNear, zFar, new Matrix4())
                : perspectiveFovRH(fov, width, height, zNear, zFar, new Matrix4());
    }

    public static Matrix4 perspectiveFov(float fov, float width, float height, float zNear, float zFar, Matrix4 mat) {
        return GLM_LEFT_HANDED ? perspectiveFovRH(fov, width, height, zNear, zFar, mat)
                : perspectiveFovLH(fov, width, height, zNear, zFar, mat);
    }

    private static Matrix4 perspectiveFovRH(float fov, float width, float height, float zNear, float zFar, Matrix4 res) {
    ...
    private static Matrix4 perspectiveFovLH(float fov, float width, float height, float zNear, float zFar, Matrix4 res) {
        float h = (float) (Math.cos(0.5f * fov) / Math.sin(0.5f * fov));
        float w = h * height / width;
        res.m00 = w;
        res.m01 = 0.0f;
